...
while IFS='|' read -r ref title period desc price tag thumb count gallery; do
  ...
  for i in "${arr[@]}"; do
    ...
    json="$json\"../assets/images/products/$FOLDER/$i\""
  done
  ...
  fi
  {
    ...
    echo "          <a class=\"catalog-row__media\" href=\"../assets/images/products/$FOLDER/$thumb\" data-lightbox=\"1\" data-lightbox-images='[$json]' data-lightbox-caption=\"$ref — $esc_title\">"
    echo "            <img src=\"../assets/images/products/$FOLDER/$thumb\" alt=\"$esc_title, reference $ref\" loading=\"lazy\">"
    ...
    echo "            <p class=\"product-price\">${price_html}<span class=\"price-indicative-tag\">indicative</span></p>"
    echo "          </div>"
    echo "        </article>"
    echo "        <!-- /$ref -->"
    echo ""
