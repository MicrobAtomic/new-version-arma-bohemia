...
4. **Parité FR/DE** : porter les sous-ensembles au niveau du catalogue anglais,
   ...
   ~~Épées~~ ✅, ~~dagues~~ ✅, ~~casques~~ ✅, ~~boucliers~~ ✅ et
   ~~rapières~~ ✅ faits (61/61, 43/43, 26/26, 3/3, 11/11 en FR et DE,
   texte repris de `armabohemia.cz/FR/` et `/DE/`, voir
   `data/fr-*.psv`/`data/de-*.psv` pour `swords`, `daggers`, `helmets`,
   `shields`, `rapiers`). Boucliers et rapières sont les deux premières
   catégories créées de zéro en FR/DE (`fr/catalogue/shields.html`,
   ...
   (rapières, armes à feu, habillement n'y apparaissaient pas du tout)
   ont été ajoutées à cette occasion. Reste à faire : les 7 autres
   catégories qui n'existent qu'en anglais pour l'instant (armures, armes
   d'hast, maroquinerie, vie de camp, vaisselle, armes à feu & arbalètes,
   habillement — lien « (EN) » dans les menus FR/DE).
